...
const Projects = () => {
  const projects = [
    ...
    },
    {
      ...
      description: "Architected a full-stack serverless application using 5 AWS Lambda functions with API Gateway and Cognito authentication, served real traffic through CloudFront CDN with S3-hosted frontend. Developed dual CI/CD pipelines with GitHub Actions, including Jest unit tests, LocalStack integration tests, linting, and security checks. Implemented in-depth security using Cognito-based authentication, customer-managed KMS encryption, and least-privilege IAM roles. Integrated automated security scanning with Checkov for IaC misconfigurations, cfn-lint for CloudFormation validation, npm audit for dependency risks, and ESLint security rules for application-level vulnerabilities. Enabled offline integration testing using LocalStack to accelerate development without incurring cloud costs.s",
